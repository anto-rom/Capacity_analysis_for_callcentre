...
"""
prophet_forecast_v5.py
Version 5 – Enhancements for Language Staffing + Full pipeline
...
def _build_capacity_backlog_with_shrink(future_df: pd.DataFrame, shrink_med_by_dow: Dict[str, float]) -> pd.DataFrame:
    ...
    for opt in CFG.WEEKDAY_AGENT_OPTIONS:
        ...
        # Simple deterministic backlog (carry positive remainder forward)
        backlog = 0.0
        backs: List[float] = []
...
def _plot_lang_agents_next30(daily_lang: pd.DataFrame, langs: List[str], out_path: str):
    df = daily_lang[['ds'] + [f'Agents_{l}' for l in langs]].copy().sort_values('ds')
    df = df.set_index('ds').head(30)  # next 30 days
...
def main():
    ...
    # CHANGED: map weekday-specific RR_hat (no global fillna)
    ...
    # 7.1) Apply +7% uplift up to Sep 30, 2025 (business rule)
    cutoff_date = pd.Timestamp("2025-09-30")
    mask = future['ds'] <= cutoff_date
    future.loc[mask, 'Total_Workload_forecast'] *= 1.07
    ...
    try:
        ...
        print(f"⚠️ Chart embedding warning (main): {e}")
    ...
    try:
        ...
        print(f"⚠️ Chart build warning (language): {e}")
    ...
    try:
        ...
        print(f"⚠️ Chart embedding warning (language): {e}")
